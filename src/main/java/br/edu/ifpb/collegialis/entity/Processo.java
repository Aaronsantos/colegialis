...
public class Processo {
	...
	public void contarVotos() {
	  ...
	  int defere = 0, indefere = 0;
	  
	  for ( Voto v  : this.votos ) {
		if(v.getVoto().equals(TipoVoto.DEFERIDO))
			defere++;
		else indefere++; 
	  ...
	  if(defere > indefere)
		  this.setDecisao(TipoDecisao.DEFERIDO);
	  else 
		  this.setDecisao(TipoDecisao.INDEFERIDO);
	}
}
