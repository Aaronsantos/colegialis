...
public class ProfessorDAO extends GenericDAO<Professor, Integer> implements Serializable{
	...
	public List<Professor> findDoCurso(String curso) {
		try {
			Query q = this.getEntityManager().createQuery("from Professor p where p.curso = :curso order by p.nome asc");
	...
	public List<Professor> consultaPorMatricula(String matricula) {
		try {
			Query q = this.getEntityManager().createQuery("from Professor a where a.matricula like :matricula order by a.nome asc");
