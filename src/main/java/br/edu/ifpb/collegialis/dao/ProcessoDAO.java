...
public class ProcessoDAO extends GenericDAO<Processo, Integer>  implements Serializable{
	...
	public List<Processo> processosAbertos() throws DAOException {
		try {
		Query q = this.getEntityManager().createQuery("from Processo p where status = NULL and reuniao = NULL  ");
	...
	@SuppressWarnings("unchecked")
	public List<Processo> findProcessoRelator(Professor prof) {
		
		Query q = this.getEntityManager().createQuery("from Processo where relator = :id");
		q.setParameter("id", prof);
		return q.getResultList();
	}
	...
	public List<Processo> findProcessoCurso(String curso) {
		CriteriaQuery<Processo> criteria = makeCriteria(curso);
		TypedQuery<Processo> query = this.getEntityManager().createQuery(criteria);
